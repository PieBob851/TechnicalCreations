...
namespace TechnicalCreations.Tiles
{
	public class ExampleDresser : ModTile
	{
		...
		public override bool RightClick(int i, int j) {
            ModContent.GetInstance<ScanBlocksUISystem>().OpenScannerUI();
		...
		public void MouseOverNearAndFarSharedLogic(Player player, int i, int j) {
			...
			if (chestIndex < 0) {
				...
				if (player.cursorItemIconText == defaultName) {
					player.cursorItemIconID = ModContent.ItemType<Items.Placeable.ExampleDresser>();
